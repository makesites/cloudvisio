...
// load a dataset
Cloudvisio.prototype.data = function( import, options ){
	// fallbacks
	options = options || {};
	...
	if( import instanceof Array){
		...
		if( options.reset ){
			...
			data = data.concat(import);
		}
	} else if( import instanceof Object) {
		// all individual imports are silent
		options.silent = true;
		// passing the key as an option - better way?
		if( options.key ){
			data[options.key] = import;
		} else {
			data.push(import);
		}
	} else {
...
// calculate the axis based on the amount of the filter keys
Cloudvisio.prototype.amount = function( axis ){
