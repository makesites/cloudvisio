...
var axis = {
	...
	add: function( axis ){
		...
		// loop through axis
		for( var value in axis[0]){
			var key = this._matchAxis(value);
			this._axis[key] = i; // customize value?
		}
	...
	// reverse method from above: match a key from the schema
	_matchAxis: function(value){
		...
		if( chart.schema.x && !axis.x ) return "x";
		// assign axis in sequence?
		return value;
	}
